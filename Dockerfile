...
RUN cd /usr/local/src; mkdir -p /usr/local/src/libcurl_static; cd /usr/local/src/libcurl_static && wget -cnv http://curl.haxx.se/download/curl-7.42.1.tar.gz && tar xzf curl-7.42.1.tar.gz && cd /usr/local/src/libcurl_static/curl-7.42.1 && ./configure --prefix=/usr/local/http2-15 --with-ssl=/usr/local/http2-15 --disable-static --enable-threaded-resolver && make && make install && echo "/usr/local/http2-15/lib" > /etc/ld.so.conf.d/curl.conf && ldconfig && echo "alias curl='/usr/local/http2-15/bin/curl'" >> /root/.bashrc && . /root/.bashrc ; alias curl='/usr/local/http2-15/bin/curl' && /usr/local/http2-15/bin/curl --version
